...
fetch('json/questions-peti.json') // Verifica que esta ruta sea correcta
    .then(response => response.json())
    .then(data => {
        if (data.length === 0) {
            console.error("El archivo 'questions-peti.json' está vacío o no tiene datos.");
        } else {
...
function saveAttempt(score) {
    ...
    // Recuperar historial de intentos de localStorage, específico para el tema PETI
    let attempts = JSON.parse(localStorage.getItem('petiAttempts')) || [];

    // Agregar el nuevo intento al historial
    attempts.push(attempt);

    // Guardar el historial actualizado en localStorage bajo la clave "petiAttempts"
    localStorage.setItem('petiAttempts', JSON.stringify(attempts));

    // Mostrar el historial de intentos en la página
    showHistory(attempts);
}

// Función para mostrar el historial de intentos (solo para PETI)
function showHistory(attempts) {
    let historyHTML = '<h3>Historial de intentos de PETI:</h3>';
...
// Mostrar historial de intentos de PETI al cargar la página
document.addEventListener('DOMContentLoaded', () => {
    const attempts = JSON.parse(localStorage.getItem('petiAttempts')) || [];
    showHistory(attempts);
});
