...
fetch('json/questions-seguridad.json') // Verifica que esta ruta sea correcta
    .then(response => response.json())
    .then(data => {
        if (data.length === 0) {
            console.error("El archivo 'questions-seguridad.json' está vacío o no tiene datos.");
        } else {
...
function saveAttempt(score) {
    ...
    // Recuperar historial de intentos de localStorage, específico para el tema Seguridad
    let attempts = JSON.parse(localStorage.getItem('seguridadAttempts')) || [];

    // Agregar el nuevo intento al historial
    attempts.push(attempt);

    // Guardar el historial actualizado en localStorage bajo la clave "seguridadAttempts"
    localStorage.setItem('seguridadAttempts', JSON.stringify(attempts));

    // Mostrar el historial de intentos en la página
    showHistory(attempts);
}

// Función para mostrar el historial de intentos (solo para Seguridad)
function showHistory(attempts) {
    let historyHTML = '<h3>Historial de intentos de Seguridad de la Información:</h3>';
...
// Mostrar historial de intentos de Seguridad al cargar la página
document.addEventListener('DOMContentLoaded', () => {
    const attempts = JSON.parse(localStorage.getItem('seguridadAttempts')) || [];
    showHistory(attempts);
});
