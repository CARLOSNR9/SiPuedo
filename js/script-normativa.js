...
fetch('questions-normativa.json') // Verifica que esta ruta sea correcta
    .then(response => response.json())
    .then(data => {
        if (data.length === 0) {
            console.error("El archivo 'questions-normativa.json' está vacío o no tiene datos.");
        } else {
...
function saveAttempt(score) {
    ...
    let attempts = JSON.parse(localStorage.getItem('attempts-normativa')) || [];

    // Agregar el nuevo intento al historial
    attempts.push(attempt);

    // Guardar el historial actualizado en localStorage
    localStorage.setItem('attempts-normativa', JSON.stringify(attempts));
...
document.addEventListener('DOMContentLoaded', () => {
    const attempts = JSON.parse(localStorage.getItem('attempts-normativa')) || [];
    showHistory(attempts);
});
