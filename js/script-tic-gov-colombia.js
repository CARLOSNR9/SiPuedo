...
fetch('questions-tic-gov-colombia.json') // Verifica que esta ruta sea correcta
    .then(response => response.json())
    .then(data => {
        if (data.length === 0) {
            console.error("El archivo 'questions-tic-gov-colombia.json' está vacío o no tiene datos.");
        } else {
...
function saveAttempt(score) {
    ...
    // Recuperar historial de intentos de localStorage, específico para el tema TIC en Gobierno
    let attempts = JSON.parse(localStorage.getItem('ticGovAttempts')) || [];

    // Agregar el nuevo intento al historial
    attempts.push(attempt);

    // Guardar el historial actualizado en localStorage bajo la clave "ticGovAttempts"
    localStorage.setItem('ticGovAttempts', JSON.stringify(attempts));

    // Mostrar el historial de intentos en la página
    showHistory(attempts);
}

// Función para mostrar el historial de intentos (solo para Implementación de TIC en el Ámbito Gubernamental)
function showHistory(attempts) {
    let historyHTML = '<h3>Historial de intentos: Implementación de TIC</h3>';
...
// Mostrar historial de intentos de TIC al cargar la página
document.addEventListener('DOMContentLoaded', () => {
    const attempts = JSON.parse(localStorage.getItem('ticGovAttempts')) || [];
    showHistory(attempts);
});
