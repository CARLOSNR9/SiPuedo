...
const topicId = 'proteccion-derechos'; // Tema actual
...
fetch('json/questions-proteccion-derechos.json')
    .then(response => response.json())
    .then(data => {
        if (data.length === 0) {
            console.error("El archivo 'questions-proteccion-derechos.json' está vacío o no tiene datos.");
        } else {
...
function saveAttempt(score) {
    const attempt = {
        ...
        score: score,
        topicId: topicId // Tema único para este simulacro
    };

    // Recuperar historial de intentos usando un nombre único por tema
