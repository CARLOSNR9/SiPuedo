...
fetch('questions-agile-methodologies.json') // Verifica que esta ruta sea correcta
    .then(response => response.json())
    .then(data => {
        if (data.length === 0) {
            console.error("El archivo 'questions-agile-methodologies.json' está vacío o no tiene datos.");
        } else {
...
function saveAttempt(score) {
    ...
    // Recuperar historial de intentos de localStorage, específico para el tema Metodologías Ágiles
    let attempts = JSON.parse(localStorage.getItem('agileAttempts')) || [];

    // Agregar el nuevo intento al historial
    attempts.push(attempt);

    // Guardar el historial actualizado en localStorage bajo la clave "agileAttempts"
    localStorage.setItem('agileAttempts', JSON.stringify(attempts));

    // Mostrar el historial de intentos en la página
    showHistory(attempts);
}

// Función para mostrar el historial de intentos (solo para Metodologías Ágiles)
function showHistory(attempts) {
    let historyHTML = '<h3>Historial de intentos de Metodologías Ágiles:</h3>';
...
// Mostrar historial de intentos de Metodologías Ágiles al cargar la página
document.addEventListener('DOMContentLoaded', () => {
    const attempts = JSON.parse(localStorage.getItem('agileAttempts')) || [];
    showHistory(attempts);
});
